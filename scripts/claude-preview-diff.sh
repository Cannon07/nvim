...
case "$TOOL_NAME" in
  Edit)
    ...
    python3 "$SCRIPT_DIR/apply-edit.py" "$FILE_PATH" "$OLD_STRING" "$NEW_STRING" "$REPLACE_ALL" "$PROP_FILE"
    ;;
  ...
  MultiEdit)
    ...
    python3 "$SCRIPT_DIR/apply-multi-edit.py" "$INPUT" "$PROP_FILE"
    ;;
...
if [[ "$HAS_NVIM" == "true" ]]; then
  ...
  nvim_send "require('custom.claude-diff').show_diff('$ORIG_ESC', '$PROP_ESC', '$DISPLAY_ESC')"
fi
